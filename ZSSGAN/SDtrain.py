...
def main():
    ...
    ######Original code for dataset loading######
    ...
    if accelerator.is_main_process:
        ...
        tracker_config.pop("clip_models")
        tracker_config.pop("clip_model_weights")
    ...
    logger.info(f"  Num examples = {100}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step in range(num_update_steps_per_epoch):
            with accelerator.accumulate(unet):
                ...
                ######Original input handling and noise prediction######
                ...
                #     # chance of setting prediction_type to sample.
                #     # Any need of keeping these parts？
                ...
                # Don't really need to care about noise residual? 
                # might just need to care about input(2text n 1image) and output(1image)
                # basically 2 text and 2 image for directional cliploss
                # Now quite clear, Instead of using the residual, use the picture itself.
                # basically use encoded image to comapare.
                # how to get image from noise residual?
                # probably use validation function to get image, then pretty much don't need to care about noise residual anymore.

                # Get the image from original model
                #为了保证两个模型采样的噪声一致，需要修改pipeline中有关噪声采样的部分，并给pipeline添加一个噪声采样输入，见stable_diffusion_pipeline.py line 777以及line 536，
                #其中prompt_embeds.dtype = torch.float32
                ...
                #定义cliploss
                clip_loss_models = {model_name: CLIPLoss(accelerator.device, 
                ...
                latents = torch.randn(shape, generator=generator, device=accelerator.device, dtype=torch.float16, layout=layout).to(accelerator.device)
                ...
                pipeline_frozen = pipeline_frozen.to(accelerator.device)
                ...
                pipeline_train = pipeline_train.to(accelerator.device)
